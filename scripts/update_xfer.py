...
ASSETS = ("VERSION", "xfer-macos-aarch64", "xfer-macos-x86_64")
...
def main() -> None:
    ...
    current_digests = {}
    for name in ASSETS:
        indent = "  " if name == "VERSION" else "      "
        pattern = rf'(?m)^{indent}url "https://github.com/cdenihan/XFER/releases/download/(v[0-9.]+)/{re.escape(name)}"\n{indent}sha256 "([0-9a-f]{{64}})"$'
    ...
    for name in ASSETS:
        old_url = f"https://github.com/cdenihan/XFER/releases/download/{current}/{name}"
        new_url = f"https://github.com/cdenihan/XFER/releases/download/{tag}/{name}"
